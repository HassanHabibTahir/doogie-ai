...
def build_fhir_bundle(patient, ai_output_text, guideline_id="NG80"):
    """
    Converts Doogie AI analysis text into FHIR + NICE compliant JSON bundle.
    """
    try:
        ai_output = json.loads(ai_output_text)
    except:
        ai_output = {"summary": ai_output_text}
            ...
            "extension": [
                {
                    ...
                    "valueInteger": patient.get("age", None)
                }
            ]
        }
    })
    ...
    # 🩺 Condition resource
    if "condition" in patient or "diagnosis" in ai_output:
        condition_name = ai_output.get("diagnosis") or patient.get("condition")
        bundle["entry"].append({
            "resource": {
                "resourceType": "Condition",
                "id": f"cond_{patient.get('id')}",
                ...
                "evidence": [
                    {"detail": [{"reference": f"NICE:{guideline_id}"}]}
                ],
                "note": [{"text": f"Based on NICE {guideline_id} guideline"}]
            }
        })

    # ⚕️ Observations (symptoms)
    for symptom in patient.get("symptoms", []):
        bundle["entry"].append({
            "resource": {
                "resourceType": "Observation",
                "id": f"obs_{symptom.lower().replace(' ', '_')}_{patient.get('id')}",
                "subject": {"reference": f"Patient/{patient.get('id')}"},
                "code": {
                    "coding": [
                        {
                ...
                "valueString": "Reported by patient"
            }
        })

    # 🧾 Provenance (audit + NICE concordance)
    provenance = {
        "resourceType": "Provenance",
        "id": f"prov_{patient.get('id')}",
        "recorded": datetime.utcnow().isoformat() + "Z",
        "target": [{"reference": f"Patient/{patient.get('id')}"}],
        "agent": [
            {"who": {"display": "Doogie AI Clinical Engine"}},
            {"who": {"display": f"NICE {guideline_id} Guideline"}}
        ],
        "entity": [
            {"role": "source", "what": {"display": f"NICE {guideline_id} Clinical Pathway"}}
        ]
    }
    ...
    if "summary" in ai_output:
        bundle["summary"] = ai_output["summary"]
...
async def upload_and_analyze(patients: list[dict] = Body(...)):
    """
    Analyze patient JSON data using Doogie system prompt and return FHIR bundle.
    Optimized to handle multiple patients concurrently.
    """
    ...
    async def process_patient(patient):
        ...
        fhir_bundle = build_fhir_bundle(patient, ai_output, guideline_id="NG80")
    ...
    # Run all patient analyses concurrently ⚡
    results = await asyncio.gather(*(process_patient(p) for p in patients))
